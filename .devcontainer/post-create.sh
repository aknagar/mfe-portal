...
# Add Dapr to PATH (bash and zsh)
echo "Configuring Dapr PATH..."
for profile in ~/.bashrc ~/.zshrc; do
    if [ -f "$profile" ] && ! grep -q ".dapr/bin" "$profile"; then
        # shellcheck disable=SC2016 # intentional: $HOME/$PATH must expand at login time, not now
        echo 'export PATH="$HOME/.dapr/bin:$PATH"' >> "$profile"
    fi
done
export PATH="$HOME/.dapr/bin:$PATH"

# Add .NET tools (aspire, etc.) to PATH (bash and zsh)
for profile in ~/.bashrc ~/.zshrc; do
