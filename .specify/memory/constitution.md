...
Changes:
- ✅ NEW: OpenSpec trial note in Development Workflow section
  Branch: 002-try-openspec
  Tooling: @fission-ai/openspec@1.2.0 (pinned) as devDependency at repo root

Template Consistency Status:
- ✅ .opencode/command/opsx-*.md - OpenSpec commands generated by openspec init
- ✅ .opencode/skills/openspec-*/SKILL.md - OpenSpec skills generated by openspec init
- ℹ️  openspec-sync-specs skill absent from this installation (not generated by init)

Follow-up Actions:
- Evaluate OpenSpec trial; if adopted, amend constitution to replace speckit workflow
- If not adopted, remove 002-try-openspec branch and revert devDependency
-->
...
> **Experimental (branch 002-try-openspec)**: An alternative spec-driven workflow using `@fission-ai/openspec@1.2.0` is under evaluation. It exposes four OpenCode commands (`/opsx-propose`, `/opsx-apply`, `/opsx-archive`, `/opsx-explore`) and mirrors the speckit workflow with a YAML-schema-driven artifact pipeline. The `build` agent (not the default `plan` agent) is required for CLI steps. This workflow does NOT replace speckit until a formal amendment is ratified.
...
**Version**: 1.1.0 | **Ratified**: 2026-01-26 | **Last Amended**: 2026-03-07
